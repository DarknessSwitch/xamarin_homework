...
namespace Books.Core.Services
{
        ...
        : IBooksService
    {
        ...
        public void GetBookItem(string id, Action<BookSearchItem> success, Action<Exception> error)
        {
            string address = string.Format("https://www.googleapis.com/books/v1/volumes/{0}",
                Uri.EscapeDataString(id));

            _simpleRestService.MakeRequest<BookSearchItem>(address, "GET", success, error);
        }
    }
}
